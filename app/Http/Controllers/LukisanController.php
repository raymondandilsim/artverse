...
class LukisanController extends Controller
{
    ...
    {
        $pagination = 9;
        $lukisans = Lukisan::Paginate($pagination);

        return view('lukisan.lihat-semua-lukisan', compact('lukisans'));
    }

    public function searchResult(Request $request)
    {
        $search = $request->get('search');
        $lukisan = Lukisan::where('nama_lukisan', 'LIKE', "%$search%")
        ->where('flag', 0)->whereHas('user', function ($query) {
                $query->where('flag', 1);
    })
    ->Paginate(6);
